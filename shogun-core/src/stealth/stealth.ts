        ...
        (Gun as any).SEA.secret(
          ...
          async (sharedSecret: string) => {
            if (!sharedSecret) {
              ...
              console.log(
                "Metodo principale fallito, tentativo alternativo...",
              );
              (Gun as any).SEA.secret(
                recipientPublicKey,
                ephemeralKeyPair.epub,
                async (altSecret: string) => {
                  if (!altSecret) {
                    reject(
                      new Error(
                        "Shared secret generation failed with all methods",
                      ),
                    );
                    return;
                  }
  ...
  async openStealthAddress(
    ...
    try {
      ...
      if (history[stealthAddress]) {
        console.log(
          "Trovati dati stealth salvati localmente:",
          history[stealthAddress],
        );
      ...
      console.log(
        "Nessun dato stealth trovato in locale, tentativo con i metodi standard",
      );
    }
