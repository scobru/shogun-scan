...
(function () {
  ...
  function attemptReconnect() {
    ...
    reconnectAttempts++;
    console.log(
      `Tentativo di riconnessione ${reconnectAttempts}/${maxReconnectAttempts}...`
    );
  ...
  function updateGunInstance() {
    ...
    if (!gun.user || !gun.SEA) {
      console.error(
        "GUN SEA non è disponibile. L'autenticazione potrebbe non funzionare."
      );
      return;
    }
  ...
  function initUserTokens() {
    ...
    tokensRef.get(userPub).once((userData) => {
      if (userData) {
        ...
        console.log(
          `Token dell'utente caricati. Bilancio: ${userTokenBalance}`
        );
    ...
    tokensRef.get(userPub).on((userData) => {
      if (
        ...
        userData.balance !== lastTokenBalance
      ) {
  ...
  function initSongCache() {
    ...
    if (!isUserAuthenticated && !isAdminAuthenticated) {
      console.log(
        "Utente non autenticato. Skipping song cache initialization."
      );
      return;
    }
  ...
  function initSongSubscription() {
    ...
    loopDetectionTimer = setInterval(() => {
      if (updatesSinceLastCheck > 20) {
        console.warn(
          "Possibile loop di aggiornamenti:",
          updatesSinceLastCheck,
          "updates in breve tempo"
        );
        clearTimeout(renderTimeout);
        isSyncPaused = true;
        setTimeout(() => {
    ...
    songsRef.map().on((song, id) => {
      ...
      if (!song || typeof song !== "object" || Object.keys(song).length === 0)
        return;
  ...
  function renderSongs() {
    ...
    if (!isUserAuthenticated && !isAdminAuthenticated) {
      songList.innerHTML =
        '<div class="alert alert-info text-center">Effettua il login per visualizzare i brani</div>';
      return;
    }
    ...
    if (Object.keys(cachedSongs).length === 0) {
      ...
      if (!songsLoaded) {
        ...
        songList.innerHTML =
          '<div class="alert alert-info text-center">Nessun brano trovato</div>';
        return;
      } else {
        console.log(
          `Caricamento iniziale terminato, ${
            Object.keys(cachedSongs).length
          } brani in cache.`
        );
      }
    }
    ...
    songIds.forEach((id) => {
      ...
      const generateSongHTML = (songData, songId) => {
        ...
        if (songData.duration) {
          ...
          durationHTML = `<div class="text-muted small">${minutes}:${seconds
            .toString()
            .padStart(2, "0")}</div>`;
        }
        ...
        const hasEditions =
          songData.mintPrice !== undefined &&
          songData.totalEditions !== undefined;
        ...
        if (hasEditions) {
          ...
          if (isAdminPage) {
            ...
            console.log(
              `Player Render Check (Song ID: ${songId}): Price=${songData.mintPrice}, Total=${songData.totalEditions}, Minted=${mintedCount}, Available=${availableEditions}, CanMint=${canMint}`
            );

            editionsHTML = `
                <div class="mt-1">
                  <span class="badge bg-info me-1">Prezzo: ${
                    songData.mintPrice
                  } token</span>
                  <span class="badge bg-secondary me-1">Edizioni: ${mintedCount}/${
              songData.totalEditions
            }</span>
                  ${
                    canMint
        ...
        if (isAdminPage) {
          ...
          if (!hasEditions) {
            console.log(
              `Admin Render Check (Song ID: ${songId}): Rendering inputs, hasEditions=false`
            );
            editionSettingsHTML = `
            ...
            console.log(
              `Admin Render Check (Song ID: ${songId}): Rendering display, hasEditions=true`
            );
        ...
        const promise = new Promise((resolve, reject) => {
          songsRef.get(id).once(
            (liveSongData) => {
              ...
              const songToRender =
                liveSongData && liveSongData.title && !liveSongData.deleted
                  ? liveSongData
                  : cachedSong;
              ...
              if (
                liveSongData &&
                JSON.stringify(liveSongData) !== JSON.stringify(cachedSong)
              ) {
              ...
              try {
                ...
                console.error(
                  `Errore generazione HTML per admin song ${id}:`,
                  e
                );
            ...
            { wait: 500 }
          ); // Aggiungi un piccolo timeout per dare tempo a Gun di rispondere
        });
    ...
    Promise.all(renderPromises)
      .then((htmlChunks) => {
        ...
        if (validHtmlChunks.length > 0) {
          ...
          songList.innerHTML =
            '<div class="alert alert-info text-center">Nessun brano valido da visualizzare</div>';
        }
        console.log(
          `Rendering completato per ${songListId}. Brani renderizzati: ${validHtmlChunks.length}/${songCount}`
        );
      })
      .catch((error) => {
        console.error("Errore durante il rendering dei brani:", error);
        songList.innerHTML =
          '<div class="alert alert-danger">Errore nel caricamento dei brani.</div>';
      });
  } // Fine renderSongs
  ...
  function playSong(songId) {
    ...
    document
      ...
      .forEach((li) => li.classList.remove("active", "bg-light"));
  ...
  async function claimDailyTokens(callback) {
    ...
    tokensRef.get(userPub).once(async (userData) => {
      ...
      if (timeSinceLastClaim < SECONDS_IN_DAY) {
        ...
        if (callback)
          callback(
            `Puoi reclamare altri token tra ${hours} ore e ${minutes} minuti`,
            null
          );
        return;
      }
      ...
      if (callback)
        callback(null, `Hai reclamato con successo ${MAX_DAILY_CLAIM} token!`);
  ...
  function setMintingDetails(songId, mintPrice, totalEditions, callback) {
    ...
    if (
      ...
      if (callback)
        callback(
          "Prezzo e numero di edizioni devono essere numeri positivi",
          null
        );
      return;
    }
    ...
    if (song.mintPrice !== undefined && song.totalEditions !== undefined) {
      if (callback)
        callback(
          "I dettagli di minting sono già stati impostati e non possono essere modificati",
          null
        );
      return;
    }
    ...
    songsRef.get(songId).once((existingSong) => {
      ...
      if (
        existingSong.mintPrice !== undefined ||
        existingSong.totalEditions !== undefined
      ) {
        if (callback)
          callback(
            "I dettagli di minting sono già stati impostati in un altro client e non possono essere modificati",
            null
          );
        return;
      }
      ...
      function tryPut() {
        retryCount++;
        console.log(
          `Tentativo di salvataggio minting details (${retryCount}/${maxRetries})`
        );
        ...
        songsRef.get(songId).put(updatedSong, (ack) => {
          if (ack.err) {
            ...
            console.log(
              "Dettagli di minting salvati con successo:",
              songId,
              updatedSong
            );
            ...
            setTimeout(() => {
              songsRef.get(songId).once((savedSong) => {
                if (
                  !savedSong ||
                  !savedSong.mintPrice ||
                  !savedSong.totalEditions
                ) {
                  console.warn(
                    "⚠️ I dettagli di minting potrebbero non essere stati salvati correttamente:",
                    savedSong
                  );
                  ...
                  if (retryCount < maxRetries) {
                    ...
                    if (callback)
                      callback(
                        "Impossibile verificare il salvataggio dei dettagli dopo multipli tentativi",
                        null
                      );
                  }
                } else {
                  console.log(
                    "✅ Dettagli di minting verificati nel database:",
                    savedSong
                  );
                  ...
                  if (callback)
                    callback(
                      null,
                      "Dettagli di minting impostati con successo e congelati"
                    );
                }
              });
            }, 1000);
  ...
  async function mintEdition(songId, callback) {
    ...
    if (!song.mintPrice || !song.totalEditions) {
      if (callback)
        callback("Questo brano non è disponibile per il minting", null);
      return;
    }
    ...
    if (userTokenBalance < song.mintPrice) {
      if (callback)
        callback(
          `Token insufficienti. Hai ${userTokenBalance}, ma servono ${song.mintPrice} token`,
          null
        );
      return;
    }
    ...
    if (creatorPub) {
      ...
      if (creatorPub === userPub) {
        console.log(
          "Nessun token distribuito: l'acquirente è anche il creatore"
        );
      } else {
        ...
        tokensRef.get(creatorPub).once((creatorData) => {
          const currentBalance =
            creatorData && creatorData.balance ? creatorData.balance : 0;
          ...
          if (creatorData) {
            ...
            console.log(
              `${creatorShare} token assegnati all'autore ${creatorPub}`
            );
          }
        });
      }
      ...
      const transactionData = {
        ...
        creator: creatorPub,
        creatorShare:
          creatorPub === userPub ? 0 : Math.floor(song.mintPrice * 0.7),
        ...
        .once((existingTx) => {
          if (!existingTx) {
            ...
            musicProtocol
              ...
              .put(transactionData);
            console.log(
              `Transazione registrata: ${song.mintPrice} token distribuiti`
            );
          }
        });
    }
    ...
    if (callback)
      callback(
        null,
        `Hai mintato con successo l'edizione #${nft.edition} di ${song.title}`
      );
  }
    ...
    try {
      ...
      for (const id in cachedSongs) {
        ...
        const matchesGenre =
          !options.genre ||
          (song.genre &&
            song.genre.toLowerCase() === options.genre.toLowerCase());

        // Controlla se corrisponde all'artista specificato
        const matchesArtist =
          !options.artist ||
          (song.artist &&
            song.artist.toLowerCase() === options.artist.toLowerCase());
  ...
  window.Protocol = {
    ...
    registerUser: (username, password, callback) => {
      ...
      user.create(username, password, (createUserAck) => {
        ...
        user.auth(username, password, (authAck) => {
          ...
          if (callback)
            callback(null, "Registrazione completata con successo!");
        });
      });
    },
      ...
      if (shogun && shogun.metamask) {
        try {
          ...
          if (result.success) {
            ...
            initSongSubscription();
            if (callback)
              callback(null, "Login con MetaMask completato con successo!");
          } else {
            if (callback)
              callback(result.error || "Login con MetaMask fallito", null);
          }
        } catch (error) {
      ...
      if (shogun && shogun.webauthn) {
        try {
          ...
          if (result.success) {
            ...
            initSongSubscription();
            if (callback)
              callback(null, "Login con WebAuthn completato con successo!");
          } else {
            if (callback)
              callback(result.error || "Login con WebAuthn fallito", null);
          }
        } catch (error) {
          if (callback) callback(error.message, null);
        }
      ...
      if (shogun && shogun.webauthn) {
        try {
          ...
          if (result.success) {
            ...
            if (callback)
              callback(
                result.error || "Registrazione con WebAuthn fallita",
                null
              );
          }
        } catch (error) {
    ...
    loginWithToken: (token, callback) => {
      ...
      if (token === METADATA_AUTH_TOKEN) {
        ...
        // Log di debug
        console.log(
          "Login admin completato. Verifica cache brani:",
          Object.keys(cachedSongs).length
        );
