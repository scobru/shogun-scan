...
let AuthenticationPage = () => {
  ...
  const handleLoginSuccess = ({ userPub, username, password }) => {
    ...
    if (!finalPassword) {
      console.error(
        "Nessuna password disponibile per l'autenticazione LoneWolf"
      );
      return;
    }
  ...
  const tryLoginWithPasswords = (username, passwords, index) => {
    if (index >= passwords.length) {
      console.log(
        'Nessuna password valida trovata, tentativo di registrazione...'
      );
    ...
    authentication.loginUser(
      ...
      ({ errMessage, success }) => {
        if (errMessage) {
          console.log(
            `Tentativo ${index + 1} fallito, provo con la prossima password...`
          );
          tryLoginWithPasswords(username, passwords, index + 1);
        } else {
          console.log(
            'Autenticazione LoneWolf completata con password salvata:',
            success
          );
        }
      }
  ...
  const handleRegistrationWithLoneWolf = (username, password) => {
    authentication.registerUser(
      ...
      async ({ errMessage, success }) => {
        if (errMessage) {
          console.error(
            'Errore durante la registrazione con LoneWolf:',
            errMessage
          );
                    ...
                    if (
                      ...
                      try {
                        ...
                        if (oldPassword && oldPassword !== password) {
                          ...
                          for (let i = 5; i > 1; i--) {
                            const prevPassword = localStorage.getItem(
                              `lonewolf_${username}_${i - 1}`
                            );
                            if (prevPassword) {
                              localStorage.setItem(
                                `lonewolf_${username}_${i}`,
                                prevPassword
                              );
                            }
                          }
                          localStorage.setItem(
                            `lonewolf_${username}_1`,
                            oldPassword
                          );
                        }
                      ...
                      try {
                        ...
                        if (
                          authentication.isAuthenticated.hasOwnProperty('value')
                        ) {
                          currentValue = authentication.isAuthenticated.value;
                          console.log(
                            'Valore attuale del Subject (tramite .value):',
                            currentValue
                          );
                        }
                      } catch (e) {
                        console.log(
                          'Impossibile leggere il valore direttamente:',
                          e
                        );
                      }
                      ...
                      // Prova ad usare checkAuth per verificare/forzare l'autenticazione
                      console.log(
                        'Tentativo di utilizzo del metodo checkAuth...'
                      );
                      try {
                        ...
                        console.log(
                          'Reindirizzamento alla home dopo checkAuth...'
                        );
                        setTimeout(() => {
                          window.location.href = '/';
                        }, 1000);
                      ...
                      console.log(
                        'Non è stato possibile forzare lo stato di autenticazione'
                      );
                      console.log(
                        'Tipo di isAuthenticated:',
                        typeof authentication.isAuthenticated
                      );
                      ...
                      console.log(
                        'Tentativo di richiamo di loginUser direttamente...'
                      );
                      authentication.loginUser(
                        { username, password },
                        ({ success }) => {
                          console.log(
                            'Risultato login diretto forzato:',
                            success
                          );
          ...
          await authentication.loginUser(
            ...
            ({ errMessage, success }) => {
              if (errMessage) {
                console.error(
                  'Errore durante il login con LoneWolf dopo la registrazione:',
                  errMessage
                );
              } else {
                console.log(
                  'Autenticazione LoneWolf completata dopo registrazione:',
                  success
                );
              }
            }
  ...
  return (
    <div class="min-h-screen bg-gray-900 text-gray-100 flex items-center justify-center p-4">
      <div class="w-full max-w-md space-y-8">
        <div class="bg-gray-900 rounded-lg shadow-xl p-6 space-y-6 ">
          {shogunSDK() && (
            <LoginWithShogunSolid
              sdk={shogunSDK()}
              ...
              showWebauthn={true}
              showStandard={true}
              buttonClassName="w-full bg-violet-600 hover:bg-violet-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200"
              inputClassName="w-full bg-gray-800 border border-gray-700 text-gray-100 rounded-lg px-4 py-2 focus:outline-none focus:border-violet-500"
              labelClassName="block text-sm font-medium text-gray-400 mb-1"
              errorClassName="text-red-500 text-sm mt-1"
              dividerClassName="my-6 flex items-center justify-center space-x-4 text-gray-500"
