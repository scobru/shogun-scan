...
for (let i = 2; i < process.argv.length; i++) {
  ...
  else if ((process.argv[i] === '-a' || process.argv[i] === '--admin-password') && i + 1 < process.argv.length) {
    cmdAdminPassword = process.argv[i + 1];
    i++; // Salta il prossimo argomento (il valore del token)
    console.log(`Token di sicurezza specificato via linea di comando`);
  }
}
...
const ADMIN_PASSWORD = cmdAdminPassword || process.env.ADMIN_PASSWORD || "thisIsTheTokenForReals2";
...
// Add static route for JS files
app.use('/shogun-music/js', express.static(path.join(__dirname, 'js')));
...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'ui/player.html'));
});

// Middleware per proteggere la pagina di admin
app.get('/admin', (req, res) => {
  // Controllo se c'è il parametro di accesso diretto (sicurezza base)
  const adminToken = req.query.token;
  
  if (adminToken === ADMIN_PASSWORD) {
    // Token valido, permetti l'accesso
    res.sendFile(path.join(__dirname, 'ui/admin.html'));
  } else {
    // Nessun token o token non valido, mostra pagina di login
...
app.post('/api/admin/validate', (req, res) => {
  ...
  // Controlla se la password corrisponde (puoi modificare questa logica)
  // Per semplicità, in questo esempio useremmo lo stesso token di sicurezza come password
  if (password === ADMIN_PASSWORD) {
    // Password corretta, restituisci il token
    res.json({ 
      success: true,
